 ...
 */
class Codabar extends \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\AbstractBarcode implements \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $printRatio)
	{
		...
		for ($i = 0; $i < $len; ++$i) {
			...
			if (!isset($chr[$code[$i]])) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODABAR barcode value "%s"', $code[$i], $code));
			}
