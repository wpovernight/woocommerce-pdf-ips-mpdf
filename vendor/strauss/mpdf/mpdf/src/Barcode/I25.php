 ...
 */
class I25 extends \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\AbstractBarcode implements \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $printRatio, $checksum)
	{
		...
		for ($i = 0; $i < $clen; $i = ($i + 2)) {
			...
			if ((!isset($chr[$charBar])) or (!isset($chr[$charSpace]))) {
				// invalid character
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid I25 barcode value "%s"', $code));
			}
