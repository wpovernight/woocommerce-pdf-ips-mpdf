 ...
 */
class EanUpc extends \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\AbstractBarcode implements \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $length)
	{
		if (preg_match('/[\D]+/', $code)) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid EAN UPC barcode value "%s"', $code));
		}
		...
		if ($upce) {
			...
			if ($invalidUpce) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\BarcodeException('UPC-A cannot produce a valid UPC-E barcode');
			}
		}
