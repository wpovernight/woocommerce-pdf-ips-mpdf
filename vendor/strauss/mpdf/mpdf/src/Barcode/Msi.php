 ...
 */
class Msi extends \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\AbstractBarcode implements \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $checksum)
	{
		...
		for ($i = 0; $i < $clen; ++$i) {
			...
			if (!isset($chr[$digit])) {
				// invalid character
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in MSI barcode value "%s"', $digit, $code));
			}
			$seq .= $chr[$digit];
		}
