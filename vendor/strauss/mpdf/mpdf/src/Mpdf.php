...
use WPO\IPS\Mpdf\Vendor\Mpdf\Config\ConfigVariables;
use WPO\IPS\Mpdf\Vendor\Mpdf\Config\FontVariables;
use WPO\IPS\Mpdf\Vendor\Mpdf\Conversion;
use WPO\IPS\Mpdf\Vendor\Mpdf\Css\Border;
use WPO\IPS\Mpdf\Vendor\Mpdf\Css\TextVars;
use WPO\IPS\Mpdf\Vendor\Mpdf\Log\Context as LogContext;
use WPO\IPS\Mpdf\Vendor\Mpdf\Fonts\MetricsGenerator;
use WPO\IPS\Mpdf\Vendor\Mpdf\Output\Destination;
use WPO\IPS\Mpdf\Vendor\Mpdf\PsrLogAwareTrait\MpdfPsrLogAwareTrait;
use WPO\IPS\Mpdf\Vendor\Mpdf\QrCode;
use WPO\IPS\Mpdf\Vendor\Mpdf\Utils\Arrays;
use WPO\IPS\Mpdf\Vendor\Mpdf\Utils\NumericString;
use WPO\IPS\Mpdf\Vendor\Mpdf\Utils\UtfString;
use WPO\IPS\Mpdf\Vendor\Mpdf\Utils\Path;
use WPO\IPS\Mpdf\Vendor\Psr\Log\NullLogger;
 ...
 * @license GPL-2.0
 */
class Mpdf implements \WPO\IPS\Mpdf\Vendor\Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Gradient
	 */
	private $gradient;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Image\Bmp
	 */
	private $bmp;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Image\Wmf
	 */
	private $wmf;
	...
	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\DirectWrite
	 */
	private $directWrite;
	...
	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Fonts\FontCache
	 */
	private $fontCache;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Fonts\FontFileFinder
	 */
	private $fontFileFinder;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Tag
	 */
	private $tag;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Barcode
	 * @todo solve Tag dependency and make private
	 */
	public $barcode;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\QrCode\QrCode
	 */
	private $qrcode;
	...
	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Color\ColorModeConverter
	 */
	private $colorModeConverter;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Color\ColorSpaceRestrictor
	 */
	private $colorSpaceRestrictor;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Hyphenator
	 */
	private $hyphenator;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Pdf\Protection
	 */
	private $protection;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Http\ClientInterface
	 */
	private $httpClient;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\File\LocalContentLoaderInterface
	 */
	private $localContentLoader;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\AssetFetcher
	 */
	...
	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Language\ScriptToLanguageInterface
	 */
	private $scriptToLanguage;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Writer\BaseWriter
	 */
	private $writer;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Writer\FontWriter
	 */
	private $fontWriter;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Writer\MetadataWriter
	 */
	private $metadataWriter;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Writer\ImageWriter
	 */
	private $imageWriter;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Writer\FormWriter
	 */
	private $formWriter;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Writer\PageWriter
	 */
	private $pageWriter;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Writer\BookmarkWriter
	 */
	private $bookmarkWriter;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Writer\OptionalContentWriter
	 */
	private $optionalContentWriter;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Writer\ColorWriter
	 */
	private $colorWriter;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Writer\BackgroundWriter
	 */
	private $backgroundWriter;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Writer\JavaScriptWriter
	 */
	private $javaScriptWriter;

	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Writer\ResourceWriter
	 */
	...
	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Container\ContainerInterface
	 */
	private $container;

	/**
	 * @param mixed[] $config
	 * @param \WPO\IPS\Mpdf\Vendor\Mpdf\Container\ContainerInterface|null $container Experimental container to override internal services
	 */
	public function __construct(array $config = [], $container = null)
	{
		$this->_dochecks();

		assert(!$container || $container instanceof \WPO\IPS\Mpdf\Vendor\Mpdf\Container\ContainerInterface);
		...
		if (file_exists($this->defaultCssFile)) {
			...
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException(sprintf('Unable to read default CSS file "%s"', $this->defaultCssFile));
		}
	...
	function _setPageSize($format, &$orientation)
	{
		if (is_string($format)) {
			...
			if (!$format[0] || !$format[1]) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Invalid page format: ' . $format[0] . ' ' . $format[1]);
			}
		...
		if ($orientation === 'p' || $orientation === 'portrait') {
			...
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Incorrect orientation: ' . $orientation);
		}
	...
	function SetDisplayMode($zoom, $layout = 'continuous')
	{
		...
		if (in_array($zoom, $allowedZoomModes, true) || is_numeric($zoom)) {
			...
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Incorrect zoom display mode: ' . $zoom);
		}
		...
		if (in_array($layout, $allowedLayoutModes, true)) {
			...
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Incorrect layout display mode: ' . $layout);
		}
	}
	...
	function SetVisibility($v)
	{
		...
		if ($v == 'printonly') {
			...
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Incorrect visibility: ' . $v);
		}
		$this->visibility = $v;
	}
	...
	function AddSpotColorsFromFile($file)
	{
		$colors = @file($file);
		if (!$colors) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("Cannot load spot colors file - " . $file);
		}
	...
	function AddFont($family, $style = '')
	{
		...
		if ($this->usingCoreFont) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("mPDF Error - problem with Font management");
		}
		...
		if (!isset($this->fontdata[$family][$stylekey]) || !$this->fontdata[$family][$stylekey]) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException(sprintf('Font "%s%s%s" is not supported', $family, $style ? ' - ' : '', $style));
		}
	...
	function SetFont($family, $style = '', $size = 0, $write = true, $forcewrite = false)
	{
		...
		if ($family == '') {
			if ($this->FontFamily) {
				...
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("No font or default font set!");
			}
		}
		...
		if (($family == 'csymbol') || ($family == 'czapfdingbats') || ($family == 'ctimes') || ($family == 'ccourier') || ($family == 'chelvetica')) {
			if ($this->PDFA || $this->PDFX) {
				if ($family == 'csymbol' || $family == 'czapfdingbats') {
					throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("Symbol and Zapfdingbats cannot be embedded in mPDF (required for PDFA1-b or PDFX/1-a).");
				}
		...
		if (!$this->onlyCoreFonts && !$this->usingCoreFont) {
			...
			if ($this->PDFA || $this->PDFX) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Core Adobe fonts cannot be embedded in mPDF (required for PDFA1-b or PDFX/1-a) - cannot use option to use core fonts.');
			}
			...
			if (!isset($this->fonts[$fontkey])) {
				...
				if (isset($this->CoreFonts[$fontkey])) {
					...
					if (!isset($cw)) {
						throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException(sprintf('Could not include font metric file "%s"', $file));
					}
					$i = count($this->fonts) + $this->extraFontSubsets + 1;
					...
					throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException(sprintf('Font %s not defined', $fontkey));
				}
			}
		...
		foreach ($this->objectbuffer as $ib => $objattr) {
			...
			if ($objattr['type'] === 'barcode') {
				...
				if (in_array($objattr['btype'], $barcodeTypes, true)) {
					...
					if (!class_exists('WPO\IPS\Mpdf\Vendor\Mpdf\QrCode\QrCode') || !class_exists('WPO\IPS\Mpdf\Vendor\Mpdf\QrCode\Output\Mpdf')) {
						throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('WPO\IPS\Mpdf\Vendor\Mpdf\QrCode package was not found. Install the package from Packagist with "composer require mpdf/qrcode"');
					}
	...
	function _SetTextRendering($mode)
	{
		if (!(($mode == 0) || ($mode == 1) || ($mode == 2))) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("Text rendering mode should be 0, 1 or 2 (value : $mode)");
		}
		$tr = ($mode . ' Tr');
	...
	function Output($name = '', $dest = '')
	{
		...
		if ($this->debug && error_get_last()) {
			...
			if (($e['type'] < 2048 && $e['type'] != 8) || (intval($e['type']) & intval(ini_get("error_reporting")))) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException(
		...
		if (($this->PDFA || $this->PDFX) && $this->encrypted) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('PDF/A1-b or PDF/X1-a does not permit encryption of documents.');
		}
		...
		if (count($this->PDFAXwarnings) && (($this->PDFA && !$this->PDFAauto) || ($this->PDFX && !$this->PDFXauto))) {
			...
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('PDFA/PDFX warnings generated. See log for further details');
		}
		...
		switch ($dest) {
			...
			case Destination::INLINE:
				...
				if ($this->debug && !$this->allow_output_buffering && ob_get_contents()) {
					throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Output has already been sent from the script - PDF file generation aborted.');
				}
			...
			case Destination::DOWNLOAD:
				...
				if (headers_sent()) {
					throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Data has already been sent to output, unable to output PDF file');
				}
			...
			case Destination::FILE:
				...
				if (!$f) {
					throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException(sprintf('Unable to create output file %s', $name));
				}
			...
			default:
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException(sprintf('Incorrect output destination %s', $dest));
		}
	...
	function _dochecks()
	{
		...
		if (1.1 == 1) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Do not alter the locale before including mPDF');
		}
		...
		if (ini_get('mbstring.func_overload')) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Mpdf cannot function properly with mbstring.func_overload enabled');
		}

		if (!function_exists('mb_substr')) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('mbstring extension must be loaded in order to run mPDF');
		}
		...
		if (!function_exists('mb_regex_encoding')) {
			...
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('mbstring extension with mbregex support must be loaded in order to run mPDF.' . $mamp);
		}
	}
	...
	{
		if ($this->PDFA || $this->PDFX) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('PDFA and PDFX do not permit transparency, so mPDF does not allow Watermarks!');
		}
	...
	function AddCIDFont($family, $style, $name, &$cw, $CMap, $registry, $desc)
	{
		...
		if (isset($this->fonts[$fontkey])) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("Font already added: $family $style");
		}
		$i = count($this->fonts) + $this->extraFontSubsets + 1;
	...
	function AddCJKFont($family)
	{
		...
		if ($this->PDFA || $this->PDFX) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("Adobe CJK fonts cannot be embedded in mPDF (required for PDFA1-b and PDFX/1-a).");
		}
		if ($family == 'big5') {
			$this->AddBig5Font();
		} elseif ($family == 'gb') {
	...
	function TableHeaderFooter($content = '', $tablestartpage = '', $tablestartcolumn = '', $horf = 'H', $level = 0, $firstSpread = true, $finalSpread = true)
	{
		if (($horf == 'H' || $horf == 'F') && !empty($content)) { // mPDF 5.7.2
			...
			if (empty($content[$firstrow])) {
				if ($this->debug) {
					throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("<tfoot> must precede <tbody> in a table");
				} else {
					return;
				}
			}
	...
	{
		if ($txt instanceof \WPO\IPS\Mpdf\Vendor\Mpdf\WatermarkText) {
	...
	{
		if ($src instanceof \WPO\IPS\Mpdf\Vendor\Mpdf\WatermarkImage) {
	...
	function WriteHTML($html, $mode = HTMLParserMode::DEFAULT_MODE, $init = true, $close = true)
	{
		...
		if (is_scalar($html) === false) {
			if (!is_object($html) || ! method_exists($html, '__toString')) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('WriteHTML() requires $html be an integer, float, string, boolean or an object with the __toString() magic method.');
			}
		}

		// Check the mode is valid
		if (in_array($mode, HTMLParserMode::getAllModes(), true) === false) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('WriteHTML() requires $mode to be one of the modes defined in HTMLParserMode');
		}
	...
	function printbuffer($arrayaux, $blockstate = 0, $is_table = false, $table_draft = false, $cell_dir = '')
	{
		...
		for ($i = 0; $i < $array_size; $i++) {
			...
			if (($blockdir == 'rtl' || $this->biDirectional) && isset($arrayaux[$i][4]) && in_array($arrayaux[$i][4], ['ccourier', 'ctimes', 'chelvetica', 'csymbol', 'czapfdingbats'])) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("You cannot use core fonts in a document which contains RTL text.");
			}
		}
		// mPDF 6
	...
	// mPDF 6
		...
		if (($toonarrow) && ($this->table_error_report)) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("Word is too long to fit in table - " . $this->table_error_report_param);
		}
		if ($toonarrow) {
			return -$biggestword;
		} else {
		...
		if (!$this->is_utf8($html)) {
			...
			while (mb_convert_encoding(mb_convert_encoding($html, "UTF-32", "UTF-8"), "UTF-8", "UTF-32") !== $html) {
				...
				if ($error && $error['message'] === 'iconv(): Detected an illegal character in input string') {
					throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Invalid input characters. Did you set $mpdf->in_charset properly?');
				}
			...
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("HTML contains invalid UTF-8 character(s). See log for further details");
		}
	...
	function purify_utf8_text($txt)
	{
		...
		if (!$this->is_utf8($txt)) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("Text contains invalid UTF-8 character(s)");
		}
	...
	function WriteBarcode($code, $showtext = 1, $x = '', $y = '', $size = 1, $border = 0, $paddingL = 1, $paddingR = 1, $paddingT = 2, $paddingB = 2, $height = 1, $bgcol = false, $col = false, $btype = 'ISBN', $supplement = '0', $supplement_code = '', $k = 1)
	{
		...
		if ($arrcode === false) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Error in barcode string: ' . $codestr);
		}
		...
		if ($supplement) { // EAN-2 or -5 Supplement
			...
			if ($supparrcode === false) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Error in barcode string (supplement): ' . $codestr . ' ' . $supplement_code);
			}

			if (strlen($supplement_code) != $supplement) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Barcode supplement incorrect: ' . $supplement_code);
			}
	...
	function transformScale($s_x, $s_y, $x = '', $y = '', $returnstring = false)
	{
		...
		if (($s_x == 0) or ( $s_y == 0)) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Please do not use values equal to zero for scaling');
		}
	...
	function AdjustHTML($html, $tabSpaces = 8)
	{
		...
		if (0 >= (int) $limit) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException(sprintf(
	...
	function OverWrite($file_in, $search, $replacement, $dest = Destination::DOWNLOAD, $file_out = "mpdf")
	{
		...
		switch ($dest) {
			...
			case Destination::FILE:
				...
				if (!$f) {
					throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Unable to create output file: ' . $file_out);
				}
