...
class ColorSpaceRestrictor
{
	 ...
	 * @param \WPO\IPS\Mpdf\Vendor\Mpdf\Color\ColorModeConverter $colorModeConverter
	 * @param int $mode
	 */
	...
	private function restrictSpotColorSpace($c, &$PDFAXwarnings = [])
	{
		if (!isset($this->mpdf->spotColorIDs[$c[1]])) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Error: Spot colour has not been defined - ' . $this->mpdf->spotColorIDs[$c[1]]);
		}
