...
use WPO\IPS\Mpdf\Vendor\Mpdf\Conversion\DecToAlpha;
use WPO\IPS\Mpdf\Vendor\Mpdf\Conversion\DecToCjk;
use WPO\IPS\Mpdf\Vendor\Mpdf\Conversion\DecToHebrew;
use WPO\IPS\Mpdf\Vendor\Mpdf\Conversion\DecToOther;
use WPO\IPS\Mpdf\Vendor\Mpdf\Conversion\DecToRoman;
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if ((isset($p['POSITION'])
			...
			if ($this->mpdf->inFixedPosBlock) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('Cannot nest block with position:fixed or position:absolute');
			}
		...
		if ($currblk['inner_width'] < $mw) {
			...
			// if ($currblk['inner_width'] < $mw) { throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("DIV is too narrow for text to fit!"); }
		}
