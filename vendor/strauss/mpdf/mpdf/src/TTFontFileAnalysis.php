...
class TTFontFileAnalysis extends TTFontFile
{
	...
	function extractCoreInfo($file, $TTCfontID = 0)
	{
		...
		if (!$this->fh) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('ERROR - Can\'t open file ' . $file);
		}
		$this->_pos = 0;
		$this->charWidths = '';
		$this->glyphPos = [];
		...
		if ($version == 0x74746366) {
			if ($TTCfontID > 0) {
				...
				if (!in_array($version, [0x00010000, 0x00020000])) {
					throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("ERROR - NOT ADDED as Error parsing TrueType Collection: version=" . $version . " - " . $file);
				}
			} else {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("ERROR - Error parsing TrueType Collection - " . $file);
			}
			...
			if (!in_array($version, [0x00010000, 0x74727565])) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("ERROR - NOT ADDED as Not a TrueType font: version=" . $version . " - " . $file);
			}
			$this->readTableDirectory(false);
		}
		...
		$format = $this->read_ushort();
		if ($format != 0 && $format != 1) { // mPDF 5.3.73
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("ERROR - NOT ADDED as Unknown name table format " . $format . " - " . $file);
		}
		$numRecords = $this->read_ushort();
		$string_data_offset = $name_offset + $this->read_ushort();
		$names = [1 => '', 2 => '', 3 => '', 4 => '', 6 => ''];
		...
		if (!$names[1] && !$psName) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException("ERROR - NOT ADDED as Could not find valid font name - " . $file);
		}
		$this->name = $psName;
		if ($names[1]) {
		...
		if ($ver_maj != 1) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('ERROR - NOT ADDED as Unknown head table version ' . $ver_maj . '.' . $ver_min . " - " . $file);
		}
		$this->fontRevision = $this->read_ushort() . $this->read_ushort();
		$this->skip(4);
		$magic = $this->read_ulong();
		if ($magic != 0x5F0F3CF5) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('ERROR - NOT ADDED as Invalid head table magic ' . $magic . " - " . $file);
		}
		$this->skip(2);
		...
		if (!$unicode_cmap_offset) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('ERROR - Font (' . $this->filename . ') NOT ADDED as it is not Unicode encoded, and cannot be used by mPDF');
		}
