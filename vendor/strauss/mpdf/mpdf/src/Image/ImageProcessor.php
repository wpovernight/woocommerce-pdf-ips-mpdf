...
namespace WPO\IPS\Mpdf\Vendor\Mpdf\Image;

use WPO\IPS\Mpdf\Vendor\Mpdf\AssetFetcher;
...
use WPO\IPS\Mpdf\Vendor\Mpdf\Gif\Gif;
use WPO\IPS\Mpdf\Vendor\Mpdf\Language\LanguageToFontInterface;
use WPO\IPS\Mpdf\Vendor\Mpdf\Language\ScriptToLanguageInterface;
...
class ImageProcessor implements \WPO\IPS\Mpdf\Vendor\Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \WPO\IPS\Mpdf\Vendor\Mpdf\Image\ImageTypeGuesser
	 */
	private $guesser;
	...
	public function getImage(&$file, $firstTime = true, $allowvector = true, $orig_srcpath = false, $interpolation = false)
	{
		...
		if (!$data) {
			...
			} catch (\WPO\IPS\Mpdf\Vendor\Mpdf\Exception\AssetFetchingException $e) {
	...
	private function gzCompress($data)
	{
		if (!function_exists('gzcompress')) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException('gzcompress is not available. install ext-zlib extension.');
		}
	...
	private function imageError($file, $firstTime, $msg)
	{
		...
		if ($firstTime && ($this->mpdf->showImageErrors || $this->mpdf->debug)) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfImageException(sprintf('%s (%s)', $msg, substr($file, 0, 256)));
		}
	...
	public function processJpg($data, $file, $firstTime, $interpolation)
	{
		...
		if ($a[2] === 'DeviceCMYK' && ($this->mpdf->restrictColorSpace === 2 || ($this->mpdf->PDFA && $this->mpdf->restrictColorSpace !== 3))) {
			...
			if (!function_exists('gd_info')) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException(sprintf('JPG image may not use CMYK color space (%s).', $file));
			}
	...
	public function processPng($data, $file, $firstTime, $interpolation)
	{
		...
		if ($firstTime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3)) {
			...
			if ($pngalpha) {
				if ($this->mpdf->PDFA) {
					throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException(sprintf('PDFA1-b does not permit images with alpha channel transparency (%s).', $file));
				}
