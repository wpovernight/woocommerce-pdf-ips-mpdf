...
use WPO\IPS\Mpdf\Vendor\Mpdf\File\LocalContentLoaderInterface;
use WPO\IPS\Mpdf\Vendor\Mpdf\File\StreamWrapperChecker;
use WPO\IPS\Mpdf\Vendor\Mpdf\Http\ClientInterface;
use WPO\IPS\Mpdf\Vendor\Mpdf\Log\Context as LogContext;
use WPO\IPS\Mpdf\Vendor\Mpdf\PsrHttpMessageShim\Request;
...
class AssetFetcher implements \WPO\IPS\Mpdf\Vendor\Psr\Log\LoggerAwareInterface
{
	...
	public function fetchDataFromPath($path, $originalSrc = null)
	{
		...
		if ($wrapperChecker->hasBlacklistedStreamWrapper($path)) {
			throw new \WPO\IPS\Mpdf\Vendor\Mpdf\Exception\AssetFetchingException('File contains an invalid stream. Only ' . implode(', ', $wrapperChecker->getWhitelistedStreamWrappers()) . ' streams are allowed.');
		}
	...
	public function fetchRemoteContent($path)
	{
		...
		try {
			...
			/** @var \WPO\IPS\Mpdf\Vendor\Mpdf\PsrHttpMessageShim\Response $response */
			...
			if (!str_starts_with((string) $response->getStatusCode(), '2')) {
				...
				if ($this->mpdf->debug) {
					throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException($message);
				}
			...
			if ($this->mpdf->debug) {
				throw new \WPO\IPS\Mpdf\Vendor\Mpdf\MpdfException($message, 0, E_ERROR, null, null, $e);
			}
